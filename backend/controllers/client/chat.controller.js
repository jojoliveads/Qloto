  ...
  try {
    ...
    const [uniqueId, sender, receiver, chatTopic] = await Promise.all([
      ...
      Host.findOne({ _id: receiverId, isBlock: false })
        .lean()
        .select("name image fcmToken chatRate agencyId"),
      ChatTopic.findOne({ _id: chatTopicId })
        .lean()
        .select("_id chatId messageCount"),
    ]);
    ...
    if (!sender) {
      ...
      return res
        .status(200)
        .json({ status: false, message: "Sender does not found." });
    }
    ...
    if (!receiver) {
      ...
      return res
        .status(200)
        .json({ status: false, message: "Receiver dose not found." });
    }
    ...
    if (!chatTopic) {
      ...
      return res
        .status(200)
        .json({ status: false, message: "ChatTopic dose not found." });
    }
    ...
    if (messageType == 2) {
      ...
      chat.message = "📸 Image";
      if (req.files?.image) {
        const imageUpload = await uploadToR2(req.files.image[0], "chat/images");
        if (imageUpload.success) {
          chat.image = imageUpload.url;
        }
      }
      ...
      chat.message = "🎤 Audio";
      if (req.files?.audio) {
        const audioUpload = await uploadToR2(req.files.audio[0], "chat/audio");
        if (audioUpload.success) {
          chat.audio = audioUpload.url;
        }
      }
    } else {
      if (req.files) deleteFiles(req.files);
    ...
    if (!isWithinFreeLimit) {
      ...
      await Promise.all([
        User.updateOne(
          ...
          { $inc: { coin: -deductedCoins, spentCoins: deductedCoins } }
        ),
...
exports.fetchChatHistory = async (req, res) => {
  try {
    ...
    const [receiver, foundChatTopic] = await Promise.all([
      Host.findOne({ _id: receiverId, isBlock: false })
        .lean()
        .select("_id audioCallRate privateCallRate"),
      ChatTopic.findOne({
        $or: [
          { senderId, receiverId },
    ...
    const [savedChatTopic, updatedReadStatus, chatHistory] = await Promise.all([
      chatTopic.save(),
      Chat.updateMany(
        { chatTopicId: chatTopic._id, isRead: false },
        { $set: { isRead: true } }
      ),
...
exports.submitChatMessage = async (req, res) => {
  try {
    ...
    const [sender, receiver, chatTopic] = await Promise.all([
      Host.findOne({ _id: senderId, isBlock: false })
        .lean()
        .select("name image"),
      User.findById({ _id: receiverId, isBlock: false })
        .lean()
        .select("name image fcmToken"),
