...
export const acceptOrDeclineWithdrawRequestForHost = createAsyncThunk(
  "api/admin/withdrawalRequest/updateHostWithdrawalStatus",
  async (payload: any) => {
    if (payload?.reason) {
      return apiInstanceFetch.patch(
        `api/admin/withdrawalRequest/updateHostWithdrawalStatus?requestId=${payload?.requestId}&hostId=${payload?.hostId}&type=${payload?.type}&reason=${payload?.reason}`
      );
    } else {
      return apiInstanceFetch.patch(
        `api/admin/withdrawalRequest/updateHostWithdrawalStatus?requestId=${payload?.requestId}&hostId=${payload?.hostId}&type=${payload?.type}`
      );
    }
  }
);
...
const withdrawalSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      acceptOrDeclineWithdrawRequestForHost.fulfilled,
      (state, action: any) => {
        state.isLoading = false;
        if (action?.payload?.status === true) {
          state.withDrawal = state?.withDrawal?.filter(
